...
namespace AD.BASE.IO
{
	public abstract class ADReader : System.IDisposable
	{
		...
		#region ADReader Abstract Methods
		...
		internal ADReader(ADSettings settings, bool readHeaderAndFooter = true)
		{
		...
		/// <summary>Allows you to enumerate over each field name. This should only be used within an ADType file.</summary>
		...
		/// <summary>Creates a new ADReader and loads the default file into it.</summary>
		public static ADReader Create()
		{
			return Create(new ADSettings());
		}

		/// <summary>Creates a new ADReader and loads a file in storage into it.</summary>
		/// <param name="filePath">The relative or absolute path of the file we want to load into the reader.</param>
		public static ADReader Create(string filePath)
		{
			return Create(new ADSettings(filePath));
		}

		/// <summary>Creates a new ADReader and loads a file in storage into it.</summary>
		/// <param name="filePath">The relative or absolute path of the file we want to load into the reader.</param>
		/// <param name="settings">The settings we want to use to override the default settings.</param>
		public static ADReader Create(string filePath, ADSettings settings)
		{
		...
		/// <summary>Creates a new ADReader and loads a file in storage into it.</summary>
		/// <param name="settings">The settings we want to use to override the default settings.</param>
		public static ADReader Create(ADSettings settings)
		{
			Stream stream = ADFile.CreateStream(settings, ADStreamEnum.FileMode.Read);
			if (stream == null)
				return null;
		...
		public static ADReader Create(byte[] bytes)
		{
			return Create(bytes, new ADSettings());
		}

		/// <summary>Creates a new ADReader and loads the bytes provided into it.</summary>
		/// <param name="settings">The settings we want to use to override the default settings.</param>
		public static ADReader Create(byte[] bytes, ADSettings settings)
		{
			Stream stream = ADFile.CreateStream(new MemoryStream(bytes), settings, ADStreamEnum.FileMode.Read);
			if (stream == null)
				return null;
		...
		internal static ADReader Create(Stream stream, ADSettings settings)
		{
			stream = ADFile.CreateStream(stream, settings, ADStreamEnum.FileMode.Read);
		...
		internal static ADReader Create(Stream stream, ADSettings settings, bool readHeaderAndFooter)
		{
			// Get the baseWriter using the given Stream.
			if (settings.format == ADStreamEnum.Format.JSON)
